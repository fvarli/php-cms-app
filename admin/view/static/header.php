...
?>
    ...
    <head>
        ...
        <link rel="stylesheet"
              href="<?= admin_public_url('vendor/jquery.tagsinput/jquery.tagsinput-revisited.min.css?=' . time()) ?>">
        <script src="https://cdn.tiny.cloud/1/xbiwycvpc2s72vwxbkjbphccz649p8xdf5tdlxjgpkrfq5fd/tinymce/5/tinymce.min.js"
...
<?php if (session('user_rank') && session('user_rank') != 3): ?>
    ...
    <div class="sidebar">
        ...
        <ul>
            <?php foreach ($menu_list as $main_url => $menu): if (!permission($menu['url'], 'show', false)) continue; ?>
                <li class="<?= (route(1) == $menu['url']) || (isset($menu['submenu']) && array_search(route(1), array_column($menu['submenu'], 'url')) !== false) ? 'active' : null ?>"> <a href="<?= admin_url($menu['url']); ?>">
                    ...
                    <?php if (isset($menu['submenu'])): ?>
                        ...
                        <ul class="sub-menu">
                            <?php foreach ($menu['submenu'] as $k => $sub_menu): if (!permission($sub_menu['url'], 'show')) continue; ?>
                                <li class="<?=route(1) == $sub_menu['url'] ? 'active' : null?>">
                                    <a href="<?= admin_url($sub_menu['url']) ?>">
                                        <?= $sub_menu['title'] ?>
                                    </a>
                               </li><?php endforeach; ?>
                        </ul>
